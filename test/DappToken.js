...
contract('DappToken', function(accounts) {
	...
	it('sets the total supply upon deployement',function() {
		return DappToken.deployed().then(function(instance){
			...
			assert.equal(adminBalance.toNumber(),1000000,'it allocates the inital suply to the admin accounts')
		});
	});
	...
	it('transfers ownership', function() {
		...
		return DappToken.deployed().then(function(instance) {
			...
			assert.equal(balance.toNumber(), 250000, ' adds the amount to the reciving amount');
			return tokenInstance.balanceOf(accounts[0]);
		}).then(function(balance){
	...
	it('approves tokens for delegated transfer',function () {
		...
		return DappToken.deployed().then(function(instance){
			...
			assert.equal(allowance.toNumber(), 100, 'stores the allowance fpr delegated transfer');
		});
	});
